...
public class PokemonScreenLAB9 extends Application {
	...
	private static final String pgif = new File("icons/pikachu.gif").toURI().toString();
	...
	double pikachuCurrentPosx=0;
	double pikachuCurrentPosy=0;
...
//		currentPosx = java,util,Random.nextInt
		int ranx = new Random().nextInt(W/STEP_SIZE) * STEP_SIZE;
		currentPosx = ranx;
		int rany = new Random().nextInt(H/STEP_SIZE) * STEP_SIZE;
		currentPosy = rany;
...
//		double pikachux = cx + avatar.getLayoutX();
//		double pikachuy = cy + avatar.getLayoutY();
		moveAvatar(x, y);
	}
